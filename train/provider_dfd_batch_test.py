...
class FrustumDataset(object):
    ...
    def __init__(self, npoints, split,
                 ...
                 overwritten_data_path=None, from_rgb_detection=False, one_hot=False,generate_database=False):
        '''
        Input:
        ...
        '''
        self.dataset_kitti = KittiDataset(root_dir='/root/frustum-pointnets_RSC/dataset/', mode='TRAIN', split=split)
            ...
            if(generate_database):
                ...
                for i in range(len(self.id_list)):
                    ...
                    for j in range(len(pc_radar)):
                        radar_mask = get_radar_mask(pc_input, pc_radar[j].reshape(-1, 3))
                ...
                with open('radar_batches_stats_'+split+'.csv',mode='w') as csv_file:
                    fieldnames= ['index_batch','radar_mask_len','radar_OI','box_present','cls_labels_len']
                    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                    writer.writeheader()
                    for i in range(len(self.index_batch)):
                        writer.writerow({'index_batch':self.index_batch[i],'radar_mask_len': self.radar_mask_len[i], 'radar_OI': self.radar_OI[i], 'box_present': self.box_present[i],'cls_labels_len':self.cls_labels_len[i]})
                self.id_list= self.index_batch
                print("id_list_filtered", len(self.id_list))
            else:
                database_infos = pandas.read_csv('/root/frustum-pointnets_RSC_RADAR_fil_PC_batch/train/radar_batches_stats_'+split+'_static_4mw.csv')
                self.id_list = database_infos['index_batch']
                self.radar_OI = database_infos['radar_OI']
                self.box_present= database_infos['box_present']
                self.batch_size = []
                j = 1
                self.idx_batch = np.unique(self.id_list)
                for i in range(1,len(self.id_list)):
                    if (self.id_list[i]==self.id_list[i-1]):
                        j=j+1
                    else:
                        self.batch_size.append(j)
                        j=1
