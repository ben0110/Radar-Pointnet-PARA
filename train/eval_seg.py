...
TRAIN_DATASET = provider.RadarDataset_seg('pc_radar_2', npoints=NUM_POINT, split='train',
...
TEST_DATASET = provider.RadarDataset_seg('pc_radar_2', npoints=NUM_POINT, split='val',
                                       rotate_to_center=False, one_hot=True, all_batches=True,
                                       translate_radar_center=False, store_data=True, proposals_3=False, no_color=True)
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            ps_list,segp_list,score_list,batch_radar_mask_set,radar_rois_param_set,ids,logits=eval_one_epoch(sess,ops,TRAIN_DATASET)
            save_data("train",ps_list,segp_list,score_list,batch_radar_mask_set,radar_rois_param_set,ids,logits)
            ps_list,segp_list,score_list,batch_radar_mask_set,radar_rois_param_set,ids,logits=eval_one_epoch(sess,ops,TEST_DATASET)
            save_data("val",ps_list,segp_list,score_list,batch_radar_mask_set,radar_rois_param_set,ids,logits)
...
def save_data(src,ps_list,segp_list,score_list,batch_radar_mask_set,radar_rois_param_set,ids,logits):
        id_data=[]
        pc_roi = []
        rot_angle=[]
        logits_roi=[]
        for i in range(len(ps_list)):
            ...
            for j in range(len(batch_radar_mask_set[i])):
                ...
                if(np.count_nonzero(labels_per_roi==1)>10):
                    ...
                    logit_roi = logits[i][pos_indices,:]
                    logits_roi.append(logit_roi)
                    id_data.append(id)
        ...
        with open(output_filename, 'wb') as fp:
            #pickle.dump(logits_roi,fp)
            pickle.dump(id_data, fp)
            pickle.dump(pc_roi,fp)
            pickle.dump(rot_angle,fp)
...
def eval_one_epoch(sess, ops,dataset):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_one_hot_vec,batch_radar_mask_list,radar_rois_param = \
            get_batch_seg(dataset, test_idxs, start_idx, end_idx,
                           NUM_POINT, NUM_CHANNEL)
    ...
    """    log_string('eval box estimation accuracy (IoU=0.5): %f' % \
               (float(iou3d_correct_cnt) / max(float(box_pred_nbr_sum), 1.0)))
    print(len(TEST_DATASET.id_list),len(seg_list),len(segp_list))"""
    """IOU3d, GT_box_list, pred_box_list = compare_box_iou( TEST_DATASET.id_list, TEST_DATASET.indice_box,
                                                        ...
                                                        center_list,
                                                        segp_list, seg_list)
    eval_per_frame(TEST_DATASET.id_list, TEST_DATASET.indice_box, ps_list, seg_list, segp_list, GT_box_list,
    ...
    return ps_list,segp_list,score_list,batch_radar_mask_set,radar_rois_param_set,dataset.id_list,logits_list
