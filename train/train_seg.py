...
def train():
    ...
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            ps_list,segp_list_train,batch_radar_mask_set,radar_rois_param_set,ids_train=train_one_epoch(sess, ops, train_writer)

            ps_list,segp_list_val,batch_radar_mask_set,radar_rois_param_set,ids_val=eval_one_epoch(sess, ops, test_writer)
            ps_list, segp_list_test, batch_radar_mask_set, radar_rois_param_set, ids_test = test_one_epoch(sess, ops,
                                                                                                         test_writer)
            ...
            if epoch == 10:
                save_data("train", segp_list_train, ids_train)
                save_data("val",  segp_list_val,ids_val)
                save_data("test",segp_list_test,ids_test )


def save_data(src, segp_list, ids):
    output_filename = "/root/frustum-pointnets_RSC_RADAR_fil_PC_batch_para/dataset/RSC/seg_per_roi_" + src +".pickle"
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_one_hot_vec,batch_radar_mask_list,radar_rois_param,id_list = \
            get_batch_seg(TRAIN_DATASET, train_idxs, start_idx, end_idx,
    ...
    log_string('eval segmentation avg class acc: %f' % \
            ...
            #iou3ds_sum = 0
            #iou3d_correct_cnt = 0
    return ps_list,pc_seg_list,batch_radar_mask_set,radar_rois_param_set,ids_list
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_one_hot_vec,batch_radar_mask_list,radar_rois_param,id_list = \
            get_batch_seg(TEST_DATASET, test_idxs, start_idx, end_idx,
                          NUM_POINT, NUM_CHANNEL)
...
def test_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_one_hot_vec, batch_radar_mask_list, radar_rois_param, id_list = \
            get_batch_seg(TEST_1_DATASET, test_idxs, start_idx, end_idx,
                          NUM_POINT, NUM_CHANNEL)
