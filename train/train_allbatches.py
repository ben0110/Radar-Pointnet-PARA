...
TEST_DATASET = provider.FrustumDataset('pc_radar_2',npoints=NUM_POINT, split='val',rotate_to_center=False, one_hot=True,all_batches = True, translate_radar_center=False, store_data=True, proposals_3 =True ,no_color=True)
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            loss,mask_loss, center_loss,heading_class_loss,size_class_loss ,heading_residual_normalized_loss,size_residual_normalized_loss ,stage1_center_loss ,corner_loss_weight,center_mask=\
                MODEL.get_loss_batch(labels_pl,batch_mask_pl ,centers_pl,
                heading_class_label_pl, heading_residual_label_pl,
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        center_boxnet,stage1_center,mask_xyz_mean,center_delta,object_point_cloud_xyz,iou2ds, iou3ds,box_pred_nbr = \
            sess.run([ops['merged'], ops['step'], ops['train_op'], ops['loss'],ops['mask_loss'],ops['center_mask'],ops['center_loss'],ops['heading_class_loss'],ops['size_class_loss'],ops['heading_residual_normalized_loss'],ops['size_residual_normalized_loss'],ops['stage1_center_loss'],ops['corner_loss_weight'],
                ops['logits'], ops['centers_pred'],ops['end_points']['center_boxnet'],ops['end_points']['stage1_center'],ops['end_points']['mask_xyz_mean'],ops['end_points']['center_delta'],ops['end_points']['object_point_cloud_xyz'],
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_sclass, batch_sres, \
        batch_rot_angle, batch_one_hot_vec, batch_mask= \
            get_batch_mask(TEST_DATASET, test_idxs, start_idx, end_idx,
                NUM_POINT, NUM_CHANNEL)
