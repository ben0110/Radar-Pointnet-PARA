...
class FrustumDataset(object):
    ...
    def __init__(self, npoints, split,
        ...
        '''
        #self.dataset_kitti = KittiDataset(root_dir='/root/3D_BoundingBox_Annotation_Tool_3D_BAT/input/NuScenes/ONE/', mode='TRAIN', split=split)
        ...
        if from_rgb_detection:
            ...
            self.id_list = np.arange(len(list))
            self.idx_batch = self.id_list
            batch_list = []
            self.radar_OI=[]
            self.batch_size = []
            for i in range(len(self.id_list)):
                pc_radar = get_radar(self.id_list[i])
                m=0
    ...
    def __getitem__(self, index):
        ...
        # load radar points
        input_radar = get_radar(self.id_list[index])
        input = get_lidar(self.id_list[index])
