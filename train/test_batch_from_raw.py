...
import provider_dfd_batch_test_raw as  provider
...
def get_session_and_ops(batch_size, num_point):
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            pointclouds_pl, one_hot_vec_pl = MODEL.placeholder_inputs(batch_size, num_point)
...
def write_detection_results_batch_3dbat(result_dir, id_list_frame,  center_list_frame, \
    ...
    i = 0
    """
    for idx in TEST_DATASET.idx_batch:
        # copy pcd file
    ...
    #generate json annotation
    annotation = []
    for j in range(len(TEST_DATASET.batch_size)):
        annotation_frame= []
        for i in range(len(center_list_frame[j])):
            h, w, l, tx, ty, tz, ry = provider.from_prediction_to_label_format(center_list_frame[j][i],
                                                                               heading_cls_list_frame[j][i], heading_res_list_frame[j][i],
                                                                               size_cls_list_frame[j][i], size_res_list_frame[j][i],
                                                                               rot_angle_list_frame[j][i])
            x = tz
            z = -ty
            y = -tx
            th = h
            tw = l
            tl = w
            annotation_frame.append({"class": "Pedestrian", "width": tw, "length": tl, "height": th, "x": x, "y": y, "z": z,
                                "rotationY": ry, "trackId": i, "frameIdx": j})

        annotation.append(annotation_frame)

    with open(orig_path+ "/annotations/LIDAR_TOP/NuScenes_ONE_annotations.txt", 'w') as outfile:
        json.dump(annotation, outfile)
...
def test(output_filename, result_dir=None):
    ...
    for i in range(len(TEST_DATASET.batch_size)):
        ...
        batch_data,batch_rot_angle, batch_one_hot_vec = \
        ...
        score_list_frames.append(score_list_frames)
