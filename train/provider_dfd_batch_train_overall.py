...
def get_radar_masks(input,input_radar):
    center_y=[]
    center_y.append(input_radar[0, 0]- ((np.tan((7.5)*np.pi/180)*2)* math.sqrt(math.pow(input_radar[0,2],2)+math.pow(input_radar[0,0],2)) + 2.0)/4 )
...
def load_GT_eval(indice):
    data_val=KittiDataset('radar_2', root_dir='/home/amben/frustum-pointnets_RSC/dataset/', mode='TRAIN', split='val')
...
class FrustumDataset(object):
    ...
    def __init__(self,radar_file, npoints, split,
        ...
        self.id_list = self.dataset_kitti.sample_id_list[60:70]
...
if __name__ == '__main__':
    ...
    median_list = []
    dataset = FrustumDataset('pc_radar_2',npoints=3500, split='train',
    rotate_to_center=False, one_hot=True,all_batches = False, translate_radar_center=False, store_data=True, proposals_3 =False ,no_color=True)
